        ...
        <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form action="{{ route('translations.store') }}" method="POST" class="space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <textarea name="values[{{ $code }}]" id="value_{{ $code }}" rows="3" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-900 focus:border-brand-500 focus:outline-none" placeholder="e.g. {{ $code == 'en' ? 'Home' : 'প্রচ্ছদ' }}" required>{{ old('values.' . $code) }}</textarea>
                    @error('values.' . $code)
                        <p class="text-sm text-rose-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
